...
curl -s -X POST "$YAKYAK_API_BASE/workflow/export-render" -H "$AUTH" -H "Content-Type: application/json" \
  -d "{\"movieId\":\"$MOVIE_ID\",\"force\":false}" >/dev/null

for _ in $(seq 1 60); do
  STATUS=$(curl -s "$YAKYAK_API_BASE/workflow/get-movie-progress/$MOVIE_ID" -H "$AUTH" \
    | python3 -c "import sys,json;ex={e['type']:e['status'] for e in json.load(sys.stdin).get('executions',[])};print(ex.get('movieConcat','waiting'))")
  echo "  render: $STATUS"
